...
//If ERASE = 1: Erase EEPROM on start up
//If ERASE = 0: Keep old memory in EEPROM
...
const int TMRL = 0xDC;                  //These two bytes load Timer0 for the
const int TMRH = 0x0B;                  // correct value to wait one second
unsigned short long ADDR = 0;           //EERPOM location for data
unsigned char ADTen;
unsigned char ADOne;
unsigned char HowMuch = 0;
unsigned short long TempSAVE = 0;
unsigned char FirstCOUNT = 0;
unsigned char XBEEON;
...
void EraseEEPROM(void);                 //Erase the memory on the EEPROM
 ...
 * 
 * Within each iteration it will collect 1 temperature reading, save the latest
 *  data, and check if it should transmit the last chunk of data from EEPROM
 * 
 * Inputs:      None
 * Outputs:     None
 ******************************************************************************/
void main(void) {
    unsigned char count;
    Initial();
    Delay10KTCYx(2000);//Wait for 10 seconds for user to send ERASE of DOWNLOAD command
    while ( 1 ) {
        //Erase the EEPROM if the "E" command is received
        if ( ERASE ) {
            EraseEEPROM();
            while( 1 ) {}   //Assume that we want to not continue recording after
                            // erase command is received
        }
        
        //If DOWNLOAD is set we are connected to a computer via FTDI and thus 
        // are not running a normal mission profile. All we want to do is get 
        // the data from memory and send it out across USART.
        if ( DOWNLOAD ) {
            T0CONbits.TMR0ON = 0;       //Turn off TMR0
            while ( 1 ) {
                R_EEPROM(ADDR,TTRAN);   //Save single page of EEPROM to RAM
                for (count=0; count<28; count+=3) {
                    FormatDATA(count, TTRAN); //Format and transmit the data just
                                        // read from EEPROM
...
//        LATAbits.LATA1 = ~LATAbits.LATA1;
        ...
        if ( (TimeStamp >= 900) && (XBEEON == 0) ) {//0x0348
            LATDbits.LATD6 = 1;
...
void Initial(void) {
    ...
    //Delay for 2 minutes before beginning mission
//    Delay10KTCYx(24000);
    ...
    //Clear bunches of stuff that should be cleared on reset
    TempSAVE = 0;
    ADDR = 0;
    ERRORFlag = 0;
    USARTFlag = 0;
    ...
    //Set up Timer0
    T0CON = 0b00000100;                 //16-bit, Fosc/4, prescaler 64
...
void InitSPI1(void) {
    unsigned char i;
    SSP1STAT = 0b10001010;                  //Data sampled at end, transition from idle
...
/******************************************************************************
 * EraseEEPROM subroutine
 *
 * This routine will erase the memory on the EEPROM
 *
 * Inputs:      None
 * Outputs:     None
 ******************************************************************************/
void EraseEEPROM(void) {
    ...
    Delay10KTCYx(30);                        //Wait for 150 ms for erase to finish
    SendBYTE('E');
    ERASE = 0;
}
        ...
        while ( 1 ) {
            ...
            for (count=0; count<28; count+=3) {
                ...
                SendBYTE('\n');         //Send spacing character
                                        ...
                                        // XBee a chance to process the data
            num += 30;                  //Increment address counter
            if ( (FirstCOUNT == 133) || (FirstCOUNT == 266) ) {
            ...
            Delay10TCYx(1);             //Wait for a little bit to give receiving
        }

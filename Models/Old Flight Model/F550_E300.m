...
%% Structure
    mFrame = 0.478; %vehicle frame mass, kg (DJI F550)
    mGear = 0.064; %vehicle landing gear mass, kg (CF legs)
    K = 6; %number of props
    mStructure = mFrame + mGear;
    
%% Power System
    mBatt = 0.845; %mass of battery, kg (Lumenier 10,000mAh)
    voltB = 14.8; %battery voltage, V (6s)
    mPower = mBatt;
    
%% Propulsion Parameters
    Dprop = 9.4*2.54/100; %prop diameter, m (Graupner Eprop)
...
%% Payload Parameters
    ...
    mSPstruct = 0.1; %mass of SP structure, kg (current estimate)
